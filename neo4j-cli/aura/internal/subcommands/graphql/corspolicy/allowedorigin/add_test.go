...
func TestAddAllowedOriginFlagsValidation(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"missing all flags": {
			executedCommand: fmt.Sprintf("graphql cors-policy allowed-origin add %s --rw", allowedOrigin),
			expectedError:   "Error: required flag(s) \"data-api-id\", \"instance-id\" not set",
		},
		"missing origin": {
			executedCommand: fmt.Sprintf("graphql cors-policy allowed-origin add --data-api-id %s --instance-id %s --rw", dataApiId, instanceId),
			expectedError:   "Error: accepts 1 arg(s), received 0",
		},
		"missing data api id flag": {
			executedCommand: fmt.Sprintf("graphql cors-policy allowed-origin add %s --instance-id %s --rw", allowedOrigin, instanceId),
			expectedError:   "Error: required flag(s) \"data-api-id\" not set",
		},
		"missing instance id flag": {
			executedCommand: fmt.Sprintf("graphql cors-policy allowed-origin add %s --data-api-id %s --rw", allowedOrigin, dataApiId),
