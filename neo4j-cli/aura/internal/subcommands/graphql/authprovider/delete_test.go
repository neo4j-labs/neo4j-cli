...
func TestDeleteAuthProvider(t *testing.T) {
	...
	helper.ExecuteCommand(fmt.Sprintf("graphql auth-provider delete %s --format json --instance-id %s --data-api-id %s --organization-id %s --project-id %s --rw --yes --force", authProviderId, instanceId, dataApiId, testOrgID, testProjectID))
...
func TestDeleteAuthProviderWithTrailingNewline(t *testing.T) {
	...
	helper.ExecuteCommand(fmt.Sprintf("graphql auth-provider delete %s\"\n\" --format json --instance-id %s --data-api-id %s --organization-id %s --project-id %s --rw --yes --force", authProviderId, instanceId, dataApiId, testOrgID, testProjectID))
...
func TestDeleteAuthProviderConfirmGate(t *testing.T) {
	...
	dataApiId := "a342b824"
	authProviderId := "87d46b4b-3bfb-4ad2-8dac-0e95cf72d39f"
	base := fmt.Sprintf("graphql auth-provider delete %s --instance-id %s --data-api-id %s --organization-id %s --project-id %s --rw", authProviderId, instanceId, dataApiId, testOrgID, testProjectID)
	confirmtest.AssertLeafGate(t, confirmtest.LeafGateCase{
		Name:          "aura graphql auth-provider delete",
		NoFlagsArgs:   base,
