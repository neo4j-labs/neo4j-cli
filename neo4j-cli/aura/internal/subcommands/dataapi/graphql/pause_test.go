...
func TestPauseGraphQLDataApi(t *testing.T) {
	...
	helper.ExecuteCommand(fmt.Sprintf("data-api graphql pause --format json --instance-id %s %s --rw", instanceId, dataApiId))
...
func TestPauseGraphQLDataApiWithTrailingNewline(t *testing.T) {
	...
	helper.ExecuteCommand(fmt.Sprintf("data-api graphql pause --format json --instance-id %s %s\"\n\" --rw", instanceId, dataApiId))
