...
func TestUpdateGraphQLDataApiFlagsValidation(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"provide only one type defs flag": {
			executedCommand: fmt.Sprintf("data-api graphql update --format json --instance-id %s --type-definitions bla --type-definitions-file blabla %s --rw", instanceId, dataApiId),
		...
		"invalid type defs": {
			executedCommand: fmt.Sprintf("data-api graphql update --format json --instance-id %s --type-definitions bla %s --rw", instanceId, dataApiId),
...
func TestUpdateGraphQLDataApiWithResponse(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"update the name": {
			...
			executeCommand:      fmt.Sprintf("data-api graphql update --format json --instance-id %s --name %s %s --rw", instanceId, name, dataApiId),
			expectedRequestBody: `{"name":"my-data-api-2"}`,
			expectedResponse:    expectedResponse,
		}, "update the password": {
			mockResponse:        mockResponse,
			executeCommand:      fmt.Sprintf("data-api graphql update --format json --instance-id %s --instance-password %s %s --rw", instanceId, instancePassword, dataApiId),
			expectedRequestBody: `{"aura_instance":{"password":"dfjglhssdopfrow"}}`,
			expectedResponse:    expectedResponse,
		}, "update the username": {
			mockResponse:        mockResponse,
			executeCommand:      fmt.Sprintf("data-api graphql update --format json --instance-id %s --instance-username %s %s --rw", instanceId, instanceUsername, dataApiId),
			expectedRequestBody: `{"aura_instance":{"username":"neo4j"}}`,
			expectedResponse:    expectedResponse,
		}, "update the password and username": {
			mockResponse:        mockResponse,
			executeCommand:      fmt.Sprintf("data-api graphql update --format json --instance-id %s --instance-password %s --instance-username %s %s --rw", instanceId, instancePassword, instanceUsername, dataApiId),
			expectedRequestBody: `{"aura_instance":{"password":"dfjglhssdopfrow","username":"neo4j"}}`,
			expectedResponse:    expectedResponse,
		}, "update the typeDefs": {
			mockResponse:        mockResponse,
			executeCommand:      fmt.Sprintf("data-api graphql update --format json --instance-id %s --type-definitions %s %s --rw", instanceId, typeDefs, dataApiId),
			expectedRequestBody: `{"type_definitions":"dHlwZS=="}`,
			expectedResponse:    expectedResponse,
		}, "update all possible values in one request": {
			mockResponse:        mockResponse,
			executeCommand:      fmt.Sprintf("data-api graphql update --format json --instance-id %s --instance-password %s --instance-username %s --type-definitions %s --name %s %s --rw", instanceId, instancePassword, instanceUsername, typeDefs, name, dataApiId),
