...
func runGrant(t *testing.T, args string, responses []struct {
	rows []map[string]any
	err  error
}) (string, string, error) {
	t.Helper()
...
func TestGrant_HappyPath_Succeeds(t *testing.T) {
	_, _, err := runGrant(t, "--role analyst --user alice", []struct {
		rows []map[string]any
		err  error
	}{
		{rows: nil, err: nil},
...
func TestGrant_HappyPath_EmitsUpdatedUserRecord(t *testing.T) {
	...
	stdout, _, err := runGrant(t, "--role analyst --user alice --format json", []struct {
		rows []map[string]any
		err  error
	}{
		{rows: nil, err: nil},
...
func TestGrant_FollowUpExecError_PropagatesError(t *testing.T) {
