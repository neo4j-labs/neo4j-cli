...
func TestList_HumanLineIncludesMetadata(t *testing.T) {
	...
	e := Entry{Time: at, Command: "neo4j-cli instance list", Invoker: "interactive", Version: "v1", Workspace: "{org}", Credential: "prod"}
	seedEntries(t, cfg, []Entry{e})

	out, err := runCmd(t, newListCmd(cfg))
	require.NoError(t, err)

	assert.Contains(t, out, "[2026-06-01T12:00:00Z] neo4j-cli instance list {invoker:interactive, workspace:{org}, credential:prod}")
}
...
func TestList_JSONNormalization(t *testing.T) {
	...
	e := Entry{Time: at, Command: "neo4j-cli instance list", Invoker: "interactive", Version: "v1"}
	seedEntries(t, cfg, []Entry{e})
