...
- `--field`/`-F` and `--raw-field`/`-f` — repeatable `key=value`. `-F` reads `@<file>` (`@-` for stdin) from a file as a string; `-f` always yields a string. In a JSON body `-F` turns a literal `true`, `false`, `null` or integer into a JSON literal, while a query parameter carries the value verbatim.
...
To set a known password, reset it then resync the stored credential:

```sh
neo4j-cli admin user set-password neo4j --new-password <s> --credential <name> --rw
neo4j-cli credential dbms remove <name>
neo4j-cli credential dbms add --name <name> --uri neo4j://localhost:7687 --username neo4j --password <s> --rw
```
