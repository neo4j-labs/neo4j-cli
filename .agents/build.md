...
# Multi-platform snapshot (all platforms)
...
Uses `changie` with a single project (`neo4j-cli`). Change files are created in `.changes/unreleased/` and tagged with a `project:` field.

**`make changelog`** opens an interactive prompt for the kind.

For non-interactive use (agents/scripts):

```bash
changie new --projects neo4j-cli --kind Patch --body "description"
```
