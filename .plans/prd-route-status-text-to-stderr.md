...
7. **`update` command plain-text status lines** — dev-build / no-stable-release / pre-swap narrative / skill-install tip print on stdout, polluting `--format json` for the not-gated cases and emitting "status noise" in plain-text mode (CLI-96).
...
- ~~F11 / CLI-96 — `update` command status lines on stdout.~~ Pulled into this PRD as Pattern G (see REQ-F-013).
...
- REQ-F-010: A `Patch`-kind changelog entry is recorded via `changie new --projects neo4j-cli --kind Patch --body "fix(cli): keep stdout clean for --format json consumers — route narration to stderr (CLI-82, CLI-95, CLI-96), TTY-gate :schema H2 headers (CLI-94)"`.
...
- REQ-F-013 (CLI-96): Five status/narration sites in `neo4j-cli/internal/subcommands/update/update.go` are routed to stderr via `fmt.Fprintln/Fprintf(cmd.ErrOrStderr(), ...)`:
  - L302 `"running a dev build, nothing to update"` — early return for dev builds. Fires regardless of format → fixes a real `--format json` pollution.
  - L326 `"no stable release published yet — pass …"` — `ErrNoStableRelease` recovery. Fires regardless of format → also fixes `--format json` pollution.
  - L434 `"Current version: %s\n"` and L435 `"Checking for updates to latest version..."` — pre-swap progress narrative, already gated by `!isStructuredFormat(...)`. Plain-text-only, moved for "status noise" consistency.
  - L475 `"Tip: install the agent skill …"` — inside the post-swap `printResult` plain-text closure. Plain-text-only, moved for the same reason.
  The plain-text closures' data lines (L358 `"Already on %s. No update needed."`, L382 `"<method> already on %s; <target> available."`, L398-402 four-line `update check` block, L470 `"Successfully updated from %s to %s"`, L473 `"Refreshed skill bundle for: %s"`) are NOT moved — they ARE the plain-text-mode data the user requested, not narration.
  Wording is preserved verbatim per the no-rewording convention.
...
**Pattern G: `update` plain-text status lines → stderr** (1 file, CLI-96)

- `neo4j-cli/internal/subcommands/update/update.go` — 5 sites:
  - L302 (top-level dev-build early return): `cmd.Println(...)` → `fmt.Fprintln(cmd.ErrOrStderr(), ...)`.
  - L326 (no-stable-release recovery branch): `cmd.Println(...)` → `fmt.Fprintln(cmd.ErrOrStderr(), ...)`.
  - L434 (pre-swap "Current version"): `cmd.Printf("Current version: %s\n", current)` → `fmt.Fprintf(cmd.ErrOrStderr(), "Current version: %s\n", current)`.
  - L435 (pre-swap "Checking for updates..."): `cmd.Println(...)` → `fmt.Fprintln(cmd.ErrOrStderr(), ...)`.
  - L475 (post-swap skill-install tip inside printResult closure): `cmd.Println(...)` → `fmt.Fprintln(cmd.ErrOrStderr(), ...)`.

  Wording preserved. The `if !isStructuredFormat(cfg.Global.Format())` gate around L434-435 stays in place — narrative still only fires when format is plain-text.

  Test harness note: existing tests at `update_test.go:115-117` set BOTH `cmd.SetOut(out)` and `cmd.SetErr(out)` to the same buffer, so combined-buffer assertions like `assert.Contains(t, out, "Checking for updates...")` keep passing after the move. To regression-pin the stream split, ADD a new test that uses separate `out` and `errBuf` buffers and asserts each of the 5 moved lines lands on the err buffer (and stdout is clean of them).
...
- `.changes/unreleased/neo4j-cli-Patch-<ts>.yaml` via `changie new --projects neo4j-cli --kind Patch --body "fix(cli): keep stdout clean for --format json consumers — route narration to stderr (CLI-82, CLI-95, CLI-96), TTY-gate :schema H2 headers (CLI-94)"`.
...
- [ ] `neo4j-cli update --format json 2>/dev/null` on a dev build prints nothing on stdout (the "running a dev build" line is on stderr); under plain-text format the line appears on stderr too. Same shape for the `no stable release published yet` branch.
- [ ] `neo4j-cli update 2>/tmp/err` (plain-text default) shows `Current version: <v>\nChecking for updates to latest version...\n` on stderr; stdout contains only the final result data (or empty for early-exit cases).
