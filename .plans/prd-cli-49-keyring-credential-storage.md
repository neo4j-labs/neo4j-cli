...
- REQ-NF-009: The `stripDBUS` helper in `test/e2e/keyring/keyring_linux_test.go` must replace `DBUS_SESSION_BUS_ADDRESS` with a syntactically valid but nonexistent socket path (e.g., `unix:path=/nonexistent-dbus-socket`) rather than omitting the variable. Omitting the variable causes godbus to fall back to `dbus-launch --autolaunch`, which blocks for ~4 minutes per subprocess call in headless environments and exhausts the `go test` 10-minute timeout before the with-daemon tests can run.
...
### CI Failures — Ubuntu Smoke Tests (With Daemon Timeout)

After the `requireDaemon` / DBUS-propagation fixes (task-022), the "Keyring smoke (with daemon)" CI step still fails with `panic: test timed out after 10m0s`. The with-daemon tests themselves never execute.

**Root cause**: `go test` in the with-daemon step runs ALL Linux smoke tests — both the no-daemon group and the with-daemon group — in sequence. The no-daemon tests call `stripDBUS(baseChildEnv(...))`, which removes `DBUS_SESSION_BUS_ADDRESS` from the subprocess env. Inside `dbus-run-session`, when `DBUS_SESSION_BUS_ADDRESS` is absent from the subprocess env, godbus (the D-Bus library used by go-keyring) falls back to spawning `dbus-launch --autolaunch`. On a headless runner this blocks for ~4 minutes per subprocess call while it tries to start a new D-Bus session. Each no-daemon test therefore takes ~4 minutes instead of <1 second. CI evidence: `TestKeyring_NoDaemon_FirstRun_WritesInsecureAndWarns` takes 240.17s and PASS; `TestKeyring_NoDaemon_ConfigSet_FailsWithNoCreds` runs for 6m0s before the suite panics. The `TestKeyring_WithDaemon_*` tests never run at all.

**Fix**: Change `stripDBUS` in `test/e2e/keyring/keyring_linux_test.go` to replace `DBUS_SESSION_BUS_ADDRESS` with an invalid but syntactically valid socket path (`unix:path=/nonexistent-dbus-socket`) instead of removing the variable. When an explicit nonexistent path is set, godbus's `net.Dial("unix", path)` returns `ENOENT` immediately — no auto-launch, no multi-minute wait. Each no-daemon subprocess call then fails in milliseconds, leaving the full time budget available for the with-daemon group.
...
- [ ] `stripDBUS` sets `DBUS_SESSION_BUS_ADDRESS=unix:path=/nonexistent-dbus-socket` rather than omitting the variable; no-daemon subprocess calls complete in under 5 seconds each when run inside `dbus-run-session`.
- [ ] All four `TestKeyring_WithDaemon_*` tests execute (not timeout-skipped) in the Ubuntu with-daemon CI step and pass.
