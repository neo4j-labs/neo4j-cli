...
- **REQ-F-018**: Commands execute **in-process** against a freshly built cobra tree per call, not by re-executing the binary. Rationale: the repo's rule is "secrets via process env, NEVER argv (world-readable via `/proc`)", and a model-supplied `--password` in `args` would land in a child's `/proc/<pid>/cmdline`. In-process also preserves `clicfg`, `clierr`, `confirm`, history and `flags.EnforceWriteGate` unchanged. **Amended in task-006**: tee and the per-command telemetry event are NOT preserved — both live in `main.go` (`handleFailure`, `clievents.Emit`), not in the tree, so a tool call records history via the root hook but emits no COMMAND event and tees nothing. Emitting from the executor would have put a live analytics service on every test's path; task-013 decides whether to emit with `invoker=agent`.
...
- **REQ-F-023**: The process `os.Stdout` is redirected to `os.Stderr` and `os.Stdin` to `os.DevNull`, so no stray direct read or write can corrupt or consume the JSON-RPC frame. **Amended in task-006**: capture and swap are one operation, `mcp.ClaimStdio()`, which returns the real streams and swaps the globals in a single call — two steps could be ordered wrongly. Consequences for task-013: the returned files must go to `mcpsdk.IOTransport`, never `mcpsdk.StdioTransport` (whose `Connect` reads the `os.Stdin`/`os.Stdout` *variables*, i.e. post-swap, so it would talk to /dev/null and stderr); and the swap is at the variable, not the fd, so a write to fd 1 that never consults `os.Stdout` still reaches the frame (`dup2` is not portable and would fight handing the transport an `*os.File`).
