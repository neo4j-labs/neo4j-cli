...
- **REQ-F-005**: Overrides come free from the existing machinery — `NEO4J_CLI_FLAG_MCP_SERVER=1` (via `FlagNameToEnv`) and `neo4j-cli config set flag.mcp-server true`. `config get`/`config list` already enumerate Registry keys (`config/get.go:67`, `clicfg.go:618`), so no new plumbing.
