...
// requireCredManagerPresent asserts that the given Credential Manager target
// exists. The target format for go-keyring entries is "<service>:<username>",
// e.g. "neo4j-cli:dbms/winsmoketest/password". Uses cmdkey /list which is
// always available on Windows without requiring C# or PowerShell compilation.
//
// Note: cmdkey does not expose the stored password value, so only existence is
// checked here. Password correctness is verified indirectly by the
// reverse-migration round-trip (password restored to JSON with the right value).
func requireCredManagerPresent(t *testing.T, target string) {
	t.Helper()
	out, err := exec.Command("cmdkey", "/list").CombinedOutput()
	require.NoError(t, err, "cmdkey /list failed: %s", out)
	require.Contains(t, string(out), target,
		"expected Credential Manager entry %q to be present", target)
}
...
func requireCredManagerAbsent(t *testing.T, target string) {
	...
	out, err := exec.Command("cmdkey", "/list").CombinedOutput()
	require.NoError(t, err, "cmdkey /list failed: %s", out)
	require.NotContains(t, string(out), target,
		"expected Credential Manager entry %q to be absent", target)
}
...
func TestKeyring_Windows_CredentialAddDoesNotStoreSecretInJSON(t *testing.T) {
	...
	requireCredManagerPresent(t, "neo4j-cli:dbms/winsmoketest/password")
...
func TestKeyring_Windows_ForwardMigration(t *testing.T) {
	...
	requireCredManagerPresent(t, "neo4j-cli:dbms/winlocaldb/password")
...
func TestKeyring_Windows_ReverseMigration(t *testing.T) {
	...
	requireCredManagerPresent(t, "neo4j-cli:dbms/winrevdb/password")
...
func TestKeyring_Windows_RemoveCleansKeyring(t *testing.T) {
	...
	require.Equal(t, 0, exitCode, "credential dbms add must succeed; stderr=%s", stderr)
	requireCredManagerPresent(t, "neo4j-cli:dbms/wintoremove/password")
