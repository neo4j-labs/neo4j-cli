...
func newKeyringTestCredentials(t *testing.T, mock *mockKeyringProvider, credentialsJSON string) (*credentials.Credentials, afero.Fs) {
	...
	creds.SetStorageMode(credentials.StorageModeKeyring, io.Discard)
	return creds, fs
}
...
func TestSetStorageMode_InsecureIsNoOp(t *testing.T) {
	...
	creds := credentials.NewCredentials(fs, clicfg.ConfigPrefix)
	creds.SetStorageMode(credentials.StorageModeInsecure, io.Discard)
...
// TestSetStorageMode_KeyringMode_MissingRequired_Warn verifies that when
// keyring mode is active and a required field is missing from both keyring and
// JSON, SetStorageMode leaves the field empty so that the command can still run.
// The missing-credential warning is written to warnW.
...
// stops for that credential (warn-and-continue).
...
func TestKeyringMode_LoadReload(t *testing.T) {
	...
	creds1.SetStorageMode(credentials.StorageModeKeyring, io.Discard)
	...
	creds2.SetStorageMode(credentials.StorageModeKeyring, io.Discard)
