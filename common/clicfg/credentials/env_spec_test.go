...
func TestValidateEmbedEnvSet(t *testing.T) {
	tests := []struct {
		name    string
		env     map[string]string
		wantErr bool
		...
		{
			"ollama needs no key",
			map[string]string{credentials.EnvEmbedProvider: "ollama"},
			false,
		},
		{
			"vertex uses ADC, needs no key var",
			map[string]string{credentials.EnvEmbedProvider: "vertex"},
			false,
		},
		{
			"openai without key",
			map[string]string{credentials.EnvEmbedProvider: "openai"},
			true,
		},
		{
			"openai with provider key",
			map[string]string{credentials.EnvEmbedProvider: "openai", credentials.EnvOpenAIKey: "sk-x"},
			false,
		},
		{
			"openai with generic key",
			map[string]string{credentials.EnvEmbedProvider: "openai", credentials.EnvEmbedAPIKey: "k"},
			false,
		},
		{
			"huggingface without key",
			map[string]string{credentials.EnvEmbedProvider: "huggingface"},
			true,
		},
		{
			"gemini with google key",
			map[string]string{credentials.EnvEmbedProvider: "gemini", credentials.EnvGoogleKey: "k"},
			false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := credentials.ValidateEmbedEnvSet(fakeGetenv(tt.env))
			if tt.wantErr {
				require.Error(t, err)
				assert.Contains(t, err.Error(), credentials.EnvEmbedProvider)
			} else {
				require.NoError(t, err)
			}
		})
