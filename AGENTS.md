...
- **Aura tier vocabulary is version-split** — the v2beta1 scoped create path (`instance create`/`deploy`/`load`) takes `free`/`professional`/`business-critical`/`virtual-dedicated-cloud`; the still-v1 `customer-managed-key create` takes the old `free-db`/`professional-db`/`enterprise-db`/`*-ds` set. Two flag types enforce this: `flags.InstanceType` normalizes legacy aliases → canonical **inside `Set()`** (so every downstream comparison — free-tier flag rules, request body, stored-credential database name — sees the canonical name), `flags.LegacyInstanceType` stays v1. Don't merge them until CMK migrates. AuraDS has no v2beta1 successor — graph analytics is now the orthogonal `graph_analytics` property (`unavailable|serverless|plugin`), so `*-ds` is rejected locally. The published `v2beta1/spec.json` RESPONSE schemas (`InstanceDetails.type`, `CreateInstanceResponse.type`) still enumerate the v1 names and are stale — trust the request examples and the live validator.
