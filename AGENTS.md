...
- **Docker** (`neo4j-cli/internal/subcommands/docker/`) — shells to host `docker`; Docker is source-of-truth (managed containers carry `org.neo4j.cli.managed=true` + metadata labels; no state file). Details:
  ...
  - `create` auto-suffixes name vs `PsAll` + `DbmsCredentials.List()`. `--ephemeral` adds `--rm`, skips cred persistence, emits `.env` blob to stdout or `--env-out-file` (0600); consumed by `query --env`. **Gap**: does NOT call `clievents.RegisterSecretValue` — under MCP, `--format table`/`toon` leaks the generated password into transcripts (JSON field regex closes it for `--format json`).
