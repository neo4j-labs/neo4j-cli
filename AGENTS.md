...
- Docker container passwords: `subcommands/docker/password.go` `generatePassword()` is the SINGLE mint point (`docker create`; `docker load` + `aura instance load` staging, both new-container path only) and registers the value itself — callers must not, and must not inline `randSource` + base64 again. An operator-supplied `--password` is deliberately NOT registered in docker: literal `ReplaceAll` of a value like `neo4j` would rewrite `neo4j://…`, `neo4j:enterprise`, `username: neo4j`. Deliberately ASYMMETRIC with `desktop/{dbms,connection}/create.go` + `connection/update.go`, which DO register operator-supplied passwords — don't generalise either side into a repo-wide rule.
